...
if __name__ == '__main__':
    #https://api.ivi.ru/light/?app_version=5462&ts=Ud-DKqMNJg8&sign=de5ef1e0b7b4ec96
    ts = 'Ud-DKqMNJg8'
    data = '{"method":"da.content.get","params":[159223,{"app_version":5462,"session":"4a39aaa34968314427848927_1541821658-0SzXiQz_MdxVN5XNpTVDOpA","site":"s959","uid":"365864.49748225","_url":"https://www.ivi.ru/watch/159223","adblock":0,"user_ab_bucket":667}]}'
    k = key.decode("hex")
    cipher = Blowfish(k)
    cipher.initCBC()
    ...
    while (offset + 8 < data_len):
        crypted = cipher.encryptCBC(data[offset : offset+8])
        print crypted.encode('hex')
        offset += 8
    ...
    sub_key = key1.decode('hex')
    if (len(buffer) < 8):
        buffer += b'\x80';
        while (len(buffer) < 8):
            buffer += b'\x00'
        sub_key = key2.decode('hex')

    print "subKey: {0}".format(sub_key.encode('hex'))
    print "buffer: {0}".format(buffer.encode('hex'))
    
    new_buffer = ''
    for i in range(8):
        new_buffer += chr(ord(buffer[i]) ^ ord(sub_key[i]))
    print "buffer: {0}".format(new_buffer.encode('hex'))

    crypted = cipher.encryptCBC(new_buffer)
    print crypted.encode('hex')
...
if __name__ == '__main1__':
    ...
    print "\tEncrypted:\t%s" % crypted.encode('hex')
    ...
    print "\tEncrypted:\t", crypted.encode('hex')
    cipher.initCTR()
